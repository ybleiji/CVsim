...
def CV_1D_dep(self):
    ...
    We are looking to the following process:
        A^+n + e^- -> B(s)
    We are reducing ion A to atom B by transfering 1 electron. 
    We assume that:
    ...
    # check if D is a float/int
    if isinstance(D, (float, int)) is False: 
        raise ValueError('D should be either a list with the values [D1,D2] or D should be a float (D1=D2)!')
    
    # check if c_init is a float or int
    if isinstance(c_init, (float, int)) is False: 
        raise ValueError('c_init should be either a list with the values [c_init_A,c_init_B] or c_init should be a float (C_init_B = 0)!')

    # check if k0 is a float
    if isinstance(k0, (float, int)) is False: 
        raise ValueError('k0 should be either a list with the values [k0_A,k0_B] or k0 should be a float (k0_B = k0_A)!')
    ...
    if self.gridtype == 'linear':
        xval = np.linspace(0, Lx, x_steps, endpoint=True) # in cm
        c_profile = c_init * np.ones(len(xval)) # in mol/cm^3
        ...
        xval = nonlingrid * Lx/nonlingrid[-1]
        xval = np.insert(xval,0,0)
        xval, xval2 = xval[:,:-1:2], xval[:,1::2]
        c_profile = c_init * np.ones(x_steps) # in mol/cm^3, zeroth entry species A, first entry species B
    ...
    dt = self.Dm/max(D)* min(dx)**2 # in s (max size of dt)
    ...
    self.dt = dt
    Dm = D/dx*dt # model diffusion coefficient times dx
    ...
    k_c = self.ECrateconst(Eapp_r,Ef0,k0,T_C,alpha) # in cm/s, reduction
    k_a = self.ECrateconst(Eapp_r,Ef0,k0,T_C,-beta) # in cm/s, oxidation
    
    # make empty array and initialize the first time step (first BC)
    c_temp = np.zeros((len(Eapp), len(xval[0]))) # c_temp[0] is c_init at the first time step
    c_temp[0] = c_profile # zeroth entry species A
    c_prev = c_temp[0]
